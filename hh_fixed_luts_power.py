...
def simulate(volt_bits: None | int = 12, gate_bits: None | int = 12, ou_tau=1, ou_sigma=5):
    ...
    # LUTS
    def v_next(v, n4, m3, h, iapp):
        v = from_fixed(v, *VOLT_TRANSFORM)
        n4 = from_fixed(n4, *GATE_TRANSFORM)
        m3 = from_fixed(m3, *GATE_TRANSFORM)
    ...
    def n4_next(v, n4): return to_fixed(gate_next(from_fixed(v, *VOLT_TRANSFORM), from_fixed(n4, *GATE_TRANSFORM)**(1/4), n_alpha, n_beta)**4, *GATE_TRANSFORM)
    def m3_next(v, m3): return to_fixed(gate_next(from_fixed(v, *VOLT_TRANSFORM), from_fixed(m3, *GATE_TRANSFORM)**(1/3), m_alpha, m_beta)**3, *GATE_TRANSFORM)
    def h_next(v, h):  return to_fixed(gate_next(from_fixed(v, *VOLT_TRANSFORM), from_fixed(h , *GATE_TRANSFORM),        h_alpha, h_beta),    *GATE_TRANSFORM)
    ###
    def initial(v, key):
...
vbit = 12
gbit = 17
v, n4, m3, h, iapp = simulate(vbit, gbit)
...
if vbit <= 12 and gbit <= 12:

    vn = jnp.zeros((1<<(vbit), 1<<(gbit)), dtype=bool).at[v, n4].set(True)
    vm = jnp.zeros((1<<(vbit), 1<<(gbit)), dtype=bool).at[v, m3].set(True)
